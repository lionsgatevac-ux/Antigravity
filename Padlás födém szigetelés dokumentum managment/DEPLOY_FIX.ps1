...
Write-Host "ℹ️  This will update the Cloud Run service with the database connection fix."
...
gcloud run deploy $SERVICE_NAME `
    ...
    --timeout 300 `
    --max-instances 10 `
    --set-env-vars "$ENV_VARS" 2>&1 | Tee-Object -FilePath "deploy_final_log.txt"

if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Deployment Failed. Check deploy_final_log.txt"
    exit 1
}
Write-Host "✅ Deployment Success! Please refresh your browser."
